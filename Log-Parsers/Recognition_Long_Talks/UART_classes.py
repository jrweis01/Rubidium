...
class TrigUARTLog(TrigLogFile):
    ...
    def run_log_analysis(self,lines):
        for line_num in xrange(len(lines)-1):
            ...
            if '28/01/2018 19:22:30.378' in line:
                line_num = line_num
            if cmd:
                ...
                #     pass
                    ...
                    mybe_no_recognition = False # maybe remove this
...
class TrigASRUARTLog(TrigUARTLog):
    matching_excel_sheet = 'UART_sTRIG_mASR'
    def __init__(self, name,Dict_Name = 'General'):
        TrigUARTLog.__init__(self, name)
    ...
    def _log_analysis_per_command(self,lines, line_num ,cmd_asr):
        ...
        for i in range(1, d):
            try:
                line = lines[line_num + i]  # check if the next line exist
                asr_result = self._get_UART_ASR_results_from_line(lines, line_num + i)
                if asr_result is not None:
                    self._handle_asr_results(asr_result,cmd_asr,line)
            except:
                break

    def _get_UART_ASR_results_from_line(self,lines,line_num):
...
class TrigASRUARTLog_LongTalks(TrigLogFile):
    _env = "UART"
