...
class LogFile(object):
	...
	def get_header_param_JBX_method(self,lines):
		''' COMMAND LINE ORDER: 0.log_file_name  1.list_file_name  2.noise_file_name  3.signal_dB  4.noise_dB  5.trig_to_ASR_delay
								6.ASR_to_trig_delay  7.board_used  8.TRIG\ASR  9.dictionary  10.counter0  11.counter1  12.trigger_phrase
								13.*approval_phrase_asr_recognition  14.*approval_phrase_trig_recognition
...
class TrigLogFile(LogFile):
	...
	def _recognize_env_and_counter(self,line,env,counter_num):
		...
		if  ((counter_num in line) or (counter_num_no_spaces in line)) and (env.lower() in line):
			return True
		elif (counter_num in line) or (counter_num_no_spaces in line):
			return True
		else:
			return False
	...
	def _is_trig_recognition(self,line):
		...
		if ((self._env.lower() in line) or (self.counter_num in line) or (counter_no_space in line)) and ("recognized"  in line)  and  ((phrase in line)  or  ("trigger" in line)) :
			return True
	...
	def _get_trig_result_from_line(self,lines,line_num):
		...
		if self._is_trig_recognition(line):
